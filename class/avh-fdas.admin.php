...
final class AVH_FDAS_Admin
{
    ...
    public function actionAdminMenu()
    {
        ...
        $this->_hooks['avhfdas_menu_general'] = add_submenu_page(
            AVH_FDAS_Define::MENU_SLUG,
            'AVH First Defense Against Spam:' . __('General Options', 'avh-fdas'),
        ...
        $this->_hooks['avhfdas_menu_3rd_party'] = add_submenu_page(
            AVH_FDAS_Define::MENU_SLUG,
            'AVH First Defense Against Spam:' . __('3rd Party Options', 'avh-fdas'),
        ...
        if (AVH_Common::getWordpressVersion() >= 3.1) {
            $this->_hooks['avhfdas_menu_ip_cache_log'] = add_submenu_page(
                AVH_FDAS_Define::MENU_SLUG,
                'AVH First Defense Against Spam:' . __('IP Cache Log', 'avh-fdas'),
        ...
        $this->_hooks['avhfdas_menu_faq'] = add_submenu_page(
            AVH_FDAS_Define::MENU_SLUG,
            'AVH First Defense Against Spam:' . __('F.A.Q', 'avh-fdas'),
            __('F.A.Q', 'avh-fdas'),
            'avh_fdas_admin',
    ...
    {
        echo '<p>' . __(
                'To check a visitor at Project Honey Pot you must enable it below, you must also have an API key. You can get an API key by signing up for free at the <a href="http://www.projecthoneypot.org/create_account.php" target="_blank">Honey Pot Project</a>. Set the options to your own liking.',
                'avh-fdas'
            );
    ...
    public function actionAjaxIpcacheLog()
    {
                ...
                if (false === $result) {
                    $x = new WP_Ajax_Response(
                        array(
                            'what' => 'ipcachelog',
                            'id'   => new WP_Error(
                                'error_updating', __('Error updating the ipcache database table.', 'avh-fdas')
                            )
                        )
                    );
                    $x->send();
                            ...
                            if (false === $result) {
                                $x = new WP_Ajax_Response(
                                    array(
                                        ...
                                        'position' => -1,
                                        'id'       => new WP_Error(
                                            'error_deleting', __('Error deleting the IP from the database table.')
                                        )
                                    )
                                );
                                $x->send();
        ...
        $x = new WP_Ajax_Response(
            array('what' => 'ipcachelog', 'position' => -1, 'id' => new WP_Error('invalid_call', __('Invalid AJAX call')))
        );
        $x->send();
    }
            ...
            if (1 == $options['general']['useipcache']) {
                ...
                if (is_object($ip_info) && 0 == $ip_info->spam) {
                    ...
                    $this->_db->updateIpCache(
                        array('ip' => $comment->comment_author_IP, 'spam' => 1, 'lastseen' => $comment_date)
                    )
                    ;
                }
            }
        ...
        foreach ($option_data as $option) {
            ...
            switch ($option[2]) {
                ...
                case 'dropdown':
                    ...
                    break;
                case 'text-color':
                    $input_type = '<input type="text" ' . (($option[3] > 50) ? ' style="width: 95%" ' : '') . 'id="' . $option[0] . '" name="' . $option[0] . '" value="' . esc_attr(
                            stripcslashes($option_actual[$section][$option_key])
                        ) . '" size="' . $option[3] . '" /><div class="box_color ' . $option[0] . '"></div>' . "\n";
                    $explanation = $option[4];
                    break;
                case 'textarea':
                    $input_type = '<textarea rows="' . $option[5] . '" ' . (($option[3] > 50) ? ' style="width: 95%" ' : '') . 'id="' . $option[0] . '" name="' . $option[0] . '" size="' . $option[3] . '" />' . esc_attr(
                            stripcslashes($option_actual[$section][$option_key])
                        ) . '</textarea>';
                    $explanation = $option[4];
                    break;
