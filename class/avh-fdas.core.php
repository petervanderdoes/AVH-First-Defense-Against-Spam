...
class AVH_FDAS_Core
{
    ...
    public function __construct()
    {
        ...
        $this->_default_options_spamhaus = array(
            'email' => 0
        );
        $this->_default_options_ipcache = array(
            'email' => 0,
            'daystokeep' => 7
        );
         ...
         * Default Data
         */
        $this->_default_data_spam = array(
            '190001' => 0
        );
        $this->_default_data_lists = array(
            'blacklist' => '',
            'whitelist' => ''
        );
        $this->_default_data = array(
            'counters' => $this->_default_data_spam,
            'lists' => $this->_default_data_lists
        );
        /**
         * Default Nonces
         */
        $this->_default_nonces_data = null;
        $this->_default_nonces = array(
            'default' => $this->_default_nonces_data
        );
        ...
        $this->_settings->storeSetting('searchengines', array(
            '0' => 'Undocumented',
            '1' => 'AltaVista',
            '2' => 'Ask',
            '3' => 'Baidu',
        ...
        if (function_exists('wp_remote_request')) {
            ...
            if (! is_wp_error($response)) {
                ...
                if (! isset($return_array['success'])) {
                    $return_array = array(
                        'Error' => array(
                            'Unknown Return' => 'Stop Forum Spam returned an unknow string: ' . var_export($response, true)
                        )
                    );
                }
            } else {
