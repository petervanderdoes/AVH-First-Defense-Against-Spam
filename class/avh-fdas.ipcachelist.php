...
<?php }
	...
	function column_ip ($ip)
	{
		...
		if ( $ip_status && 'all' != $ip_status ) { // not looking at all spam
			$actions['spam']="<a href='$spam_url' class='delete:the-ipcache-list:ip-$ip->ip:e7e7d3:a=hs&amp;ns=1' title='" . esc_attr__('Mark this IP as spam', 'avh-fdas') . "'>" . __('Spam', 'avh-fdas') . '</a>';
			$actions['ham'] = "<a href='$ham_url' class='delete:the-ipcache-list:ip-$ip->ip:e7e7d3:a=hs&amp;ns=0' title='" . esc_attr__('Mark this IP as ham', 'avh-fdas') . "'>" . __('Ham', 'avh-fdas') . '</a>';
		} else {
		...
		$actions['blacklist'] = "<a href='$blacklist_url' class='delete:the-ipcache-list:ip-$ip->ip:e7e7d3:a=bl delete' title='" . esc_attr__('Blacklist this IP', 'avh-fdas') . "'>" . __('Blacklist', 'avh-fdas') . '</a>';
		$actions['delete'] = "<a href='$delete_url' class='delete:the-ipcache-list:ip-$ip->ip::action=test&amp;a=dl delete' title='" . esc_attr__('Delete this IP', 'avh-fdas') . "'>" . __('Delete', 'avh-fdas') . '</a>';
		$i = 0;
