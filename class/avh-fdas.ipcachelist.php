...
class AVH_FDAS_IPCacheList extends WP_List_Table {
	...
	function column_added($ip) {
		$date = mysql2date(get_option('date_format'), $ip->added) . ' at ' . mysql2date(get_option('time_format'),
		                                                                                $ip->added);
		echo $date;
	}
	...
	function column_ip($ip) {
		...
		if ($ip_status && 'all' != $ip_status) { // not looking at all spam
			$actions['spam'] = "<a href='$spam_url' class='delete:the-ipcache-list:ip-$ip->ip:e7e7d3:f=hs&amp;ns=1' title='" .
			                   esc_attr__('Mark this IP as spam',
			                              'avh-fdas') .
			                   "'>" .
			                   __('Spam', 'avh-fdas') .
			                   '</a>';
			$actions['ham']  = "<a href='$ham_url' class='delete:the-ipcache-list:ip-$ip->ip:e7e7d3:f=hs&amp;ns=0' title='" .
			                   esc_attr__('Mark this IP as ham',
			                              'avh-fdas') .
			                   "'>" .
			                   __('Ham', 'avh-fdas') .
			                   '</a>';
		} else {
			$actions['spam'] = "<a href='$spam_url' class='dim:the-ipcache-list:ip-$ip->ip:spammed:e7e7d3:e7e7d3:new_status=1' title='" .
			                   esc_attr__('Mark this IP as spam',
			                              'avh-fdas') .
			                   "'>" .
			                   __('Spam', 'avh-fdas') .
			                   '</a>';
			$actions['ham']  = "<a href='$ham_url' class='dim:the-ipcache-list:ip-$ip->ip:spammed:e7e7d3:e7e7d3:new_status=0' title='" .
			                   esc_attr__('Mark this IP as ham',
			                              'avh-fdas') .
			                   "'>" .
			                   __('Ham', 'avh-fdas') .
		...
		$actions['blacklist'] = "<a href='$blacklist_url' class='delete:the-ipcache-list:ip-$ip->ip:e7e7d3:f=bl delete' title='" .
		                        esc_attr__('Blacklist this IP',
		                                   'avh-fdas') .
		                        "'>" .
		                        __('Blacklist', 'avh-fdas') .
		                        '</a>';
		$actions['delete']    = "<a href='$delete_url' class='delete:the-ipcache-list:ip-$ip->ip::f=dl delete' title='" .
		                        esc_attr__('Delete this IP',
		                                   'avh-fdas') .
		                        "'>" .
		                        __('Delete', 'avh-fdas') .
		                        '</a>';
	...
	function column_lastseen($ip) {
		$date = mysql2date(get_option('date_format'), $ip->lastseen) . ' at ' . mysql2date(get_option('time_format'),
		                                                                                   $ip->lastseen);
		echo $date;
	}
	...
	function get_views() {
		...
		$stati        = array(
			...
			'ham'  => _n_noop('Ham <span class="count">(<span class="ham-count">%s</span>)</span>',
			                  'Ham <span class="count">(<span class="ham-count">%s</span>)</span>'),
			'spam' => _n_noop('Spam <span class="count">(<span class="spam-count">%s</span>)</span>',
		...
		foreach ($stati as $status => $label) {
			 ...
			 */
			$status_links[ $status ] = "<a href='$link'$class>" .
			                           sprintf(translate_nooped_plural($label, $num_ips->$status),
	...
	function prepare_items() {
		...
		if (isset($_REQUEST['number'])) {
			...
			$number = $ips_per_page + min(8,
			                              $ips_per_page); // Grab a few extra, when changing the 8 changes are need in avh-fdas.ipcachelist.js
		}
