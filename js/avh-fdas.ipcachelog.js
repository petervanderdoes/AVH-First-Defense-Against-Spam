...
(function ($) {
  setIpCacheLogList = function () {
    ...
    delAfter = function (r, settings) {
      ...
      if (( 'object' == typeof r ) &&
        lastConfidentTime <
        settings.parsed.responses[0].supplemental.time) {
        ...
        if (total_items_i18n) {
          ...
          $('.tablenav-pages').find('.next-page, .last-page').toggleClass('disabled', settings.parsed.responses[0].supplemental.total_pages ==
            $('.current-page').val());
        }
  ...
  $(document).ready(function () {
    setIpCacheLogList();
    $(document).delegate('span.avhfdas_is_delete a.avhfdas_is_delete', 'click', function () {
      return false;
    });
  });
})(jQuery);
